	...
	},	
{
	...
	}, 
 {
		name = "Force Heal",
		icon = "H",
		image = "lbf/forceicons/heal",
		target = 1,
		description = "Heals your target.",
		action = function( self )
			if ( self:GetForce() < 1 --[[|| !self.Owner:IsOnGround()]] or self.Owner:Health() >= self.Owner:GetMaxHealth() or CLIENT ) then return end
	...
	}, 
 {
		name = "Group Heal",
		icon = "GH",
		image = "lbf/forceicons/group_heal",
		description = "Heals all around you.",
		action = function( self )
			if ( self:GetForce() < 75 || CLIENT ) then return end
			if self.NextGroup and self.NextGroup >= CurTime() then return end
			local players = 0
			for _, ply in pairs( ents.FindInSphere( self.Owner:GetPos(), 200 ) ) do
				...
				ply:SetHealth( math.min( ply:Health() + 500, ply:GetMaxHealth() ) )
				local ed = EffectData()
			...
			self.NextGroup = CurTime() + 20
			self:SetNWFloat( "Group Heal", self.NextGroup )
		end
	}, 
 {
		name = "Cloak",
		icon = "C",
		image = "lbf/forceicons/cloak",
		description = "Shrowd yourself with the force for 10 seconds",
		action = function( self )
		if ( self:GetForce() < 50 || !self.Owner:IsOnGround() || CLIENT ) then return end
	...
	},
 {
		name = "Force Reflect",
		icon = "FR",
		image = "lbf/forceicons/reflect",
		description = "An eye for an eye",
		action = function( self )
		if ( self:GetForce() < 50 || !self.Owner:IsOnGround() || CLIENT ) then return end
			if self.Owner:GetNWFloat( "ReflectTime", 0 ) >= CurTime() then return end
			if self.NextReflect and self.NextReflect >= CurTime() then return end
			self:SetForce( self:GetForce() - 50 )
			self:SetNextAttack( 0.7 )
			self:PlayWeaponSound( "lightsaber/force_leap.wav" )
			self.Owner:SetNWFloat( "ReflectTime", CurTime() + 2 )
			self.NextReflect = CurTime() + 120
			self:SetNWFloat( "Force Reflect", self.NextReflect )
		end
	},
{
		name = "Rage",
		icon = "RA",
		image = "lbf/forceicons/rage",
		description = "Unleash your anger",
		action = function( self )
		if ( self:GetForce() < 50 || !self.Owner:IsOnGround() || CLIENT ) then return end
			if self.Owner:GetNWFloat( "RageTime", 0 ) >= CurTime() then return end
			if self.NextRage and self.NextRage >= CurTime() then return end
			self:SetForce( self:GetForce() - 50 )
			self:SetNextAttack( 0.7 )
			self:PlayWeaponSound( "lightsaber/force_leap.wav" )
			self.Owner:SetNWFloat( "RageTime", CurTime() + 10 )
			self.NextRage = CurTime() + 60
			self:SetNWFloat( "Rage", self.NextRage )
		end
	},
 {
		name = "Shadow Strike",
		icon = "SS",
		distance = 30,
		image = "lbf/forceicons/shadow_strike",
		target = 1,
		...
		action = function( self )
			...
			if self.NextShadow and self.NextShadow >= CurTime() then return end
			self:SetForce( self:GetForce() - 50 )
			...
			self.NextShadow = CurTime() + 20
			self.Owner:SetNWFloat( "CloakTime", 0 )
			self:SetNWFloat( "Shadow Strike", self.NextShadow )
		end
	},
{
		...
		icon = "LS",
		distance = 600,
		image = "lbf/forceicons/lightning_strike",
		target = 1,
		...
		action = function( self )
			...
			self.NextStrike = CurTime() + 6
			self:SetNWFloat( "Lightning Strike", self.NextStrike )
		end
	},
{
		name = "Advanced Cloak",
		icon = "AC",
		image = "lbf/forceicons/advanced_cloak",
	...
	},
 {
		...
		icon = "L",
		target = 1,
		image = "lbf/forceicons/lightning",
	...
	},
 {
		...
		action = function( self )
			...
			if self.NextStorm and self.NextStorm >= CurTime() then return end
			if SERVER then
			...
			self.NextStorm = CurTime() + 120
			self:SetNWFloat( "Storm", self.NextStorm )
		end
	},
}
