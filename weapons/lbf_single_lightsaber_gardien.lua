...
SWEP.PrintName = "Sabre Laser de Gardien Jedi"
...
{
		name = "Barrage de Force",
		icon = "BF",
		description = "A utiliser pour vous protéger !",
		action = function( self )
			if ( self:GetForce() < 1/* || !self.Owner:IsOnGround()*/ || CLIENT ) then return end
			self:SetForce( self:GetForce() - 0.1 )
			self.Owner:SetNWFloat( "AbsorbTime", CurTime() + 1 )
...
function SWEP:Deploy()
	...
	self:SetMaxLength( 42 )
	self:SetCrystalColor( Vector( 0 , 63 , 255 ) )
